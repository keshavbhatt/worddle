...
RateApp::RateApp(QWidget *parent, QString app_rating_url, int app_launch_count,
  ...
  this->app_launch_count = app_launch_count; // How many time the app must be
                                             // launched by user to show this
                                             // dialog
  this->app_install_days = app_install_days; // How many days the app must be
                                             // installed by user to show this
                                             // dialog
  this->present_delay = present_delay; // Delay after which this dialog should
                                       // be shown to use if all conditions
                                       // matched
...
void RateApp::on_rateOnGithub_clicked() {
  QDesktopServices::openUrl(QUrl("https://github.com/keshavbhatt/worddle"));
  this->reset();
  this->close();
}

void RateApp::on_donate_clicked() {
  QDesktopServices::openUrl(QUrl("https://paypal.me/keshavnrj/3.00"));
  this->reset();
  this->close();
}
